...
ListView createAnnouncementList(List<AnnouncementData> announcements, List<String> selectedClasses,
  ...
  ListView.builder(
    ...
    itemBuilder: (context, index) {
      if (index == 0) {
        return Text("UNCOMPLETED", style: TextStyle(fontWeight: FontWeight.bold));
      }
      index--;

      if (announcements.isEmpty ||
        (index == 0 && announcements.isNotEmpty && announcements[index].isCompleted()) ||
        (index - 1 < announcements.length && index - 1 >= 0 &&
        !announcements[index - 1].isCompleted() && (
          (index < 0 || index >= announcements.length) || announcements[index].isCompleted()
        ))) {
        return Text("COMPLETED", style: TextStyle(fontWeight: FontWeight.bold));
      }
  ...
  return AlertDialog(
    ...
    content: Column(
      ...
      children: [
        ...
        Row(children: [Text("Posted by: ",  style: TextStyle(fontWeight: FontWeight.bold)),
                                            Text(data.getAuthor())]),
        Row(children: [Text("Due: ",        style: TextStyle(fontWeight: FontWeight.bold)),
                                            Text(data.getDue())]),
        Row(children: [Text("Type: ",       style: TextStyle(fontWeight: FontWeight.bold)),
    ...
    actions: [
      TextButton(
          ...
          if (!data.isCompleted()) {
            if (await data.complete()) {
              if (context.mounted) {
                ...
                showSnackBar(context, "Announcement Completed. (\"${data.getTitle()}\")");
              }
        ...
        child: Text(data.isCompleted() ? "Completed" : "Complete", style: TextStyle(
          fontWeight: FontWeight.bold,
      ...
      TextButton(
                    ...
                    onPressed: () async {
                      if (await deleteAnnouncementFromServer(data)) {
                        if (context.mounted) {
                          Navigator.pop(context);
                          Navigator.pop(context);
                          showSnackBar(context, "Successfully deleted announcement. (\"${data.getTitle()}\")");
                        }
        ...
        child: Text("Delete", style: TextStyle(
          fontWeight: FontWeight.bold,
          color: data.getAuthorUUID() == account.uuid ? Colors.red : Colors.grey
        )),
      ),
      TextButton(
        onPressed: () => Navigator.pop(context),
